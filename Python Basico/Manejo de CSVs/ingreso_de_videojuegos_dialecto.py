...
def save_to_tsv(path, data): #Path y data como parámetros () (TSV: Tab-Separated Values)
    with open(path, 'w', encoding="utf-8", newline='') as file: #Con el archivo definido en path abierto como file:
        writer = csv.writer(file, dialect="excel-tab") #En este caso, se utilizó el dialecto excel-tab para separar los datos, sin embargo, se puede usar el parámetro delimiter = "\t"
...
def main():
    ...
    save_to_tsv("Manejo de Archivos/Lista_de_videojuegos.tsv", name) #path y data en ese orden, llamamos a la función save_to_tsv (la extensión del archivo cambia a tsv)
main()